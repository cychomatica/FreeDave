# built on https://github.com/Gen-Verse/dLLM-RL/blob/main/reward/execute.py
...
def excute_main(outputs_dir, outputs_filename, num_chunk):
    ...
    # --- 1) function ---
    if func_items:
        updated_func = evaluate_function_dataset(func_items, n_workers=num_chunk)
    ...
    if stdio_items:
        ...
        num_chunks    = max(1, math.ceil(total_scripts / num_chunk))
    ...
    # --- save JSON ---
    with open(os.path.join(outputs_dir, outputs_filename), "w", encoding="utf-8", errors="surrogatepass") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--outputs_dir', type=str, required=True, help='Directory containing outputs')
    parser.add_argument('--outputs_filename', type=str, required=True, help='Filename of outputs JSON')
    parser.add_argument('--num_chunk', type=int, default=8, help='Number of chunks/workers')
    args = parser.parse_args()

    excute_main(args.outputs_dir, args.outputs_filename, args.num_chunk)
