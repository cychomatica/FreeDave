...
# Last constructed Dream LM (per process). Used to merge generation stats into lm_eval JSON.
_ACTIVE_DREAM_LM: Optional["Dream"] = None
...
class Dream(LM):
    ...
    def _emit_inference_stats_for_eval(
        ...
        """Merge Dream generation stats into lm_eval output: dict key, log line, optional sidecar JSON.
        ...
        if (
            ...
            lm_eval_results["dream_inference_stats"] = payload

        if getattr(inst, "_print_inference_stats_at_exit", True):
            eval_logger.info(
                "Dream inference stats (summary): %s",
        ...
        if path:
            ...
            eval_logger.info("Wrote Dream inference stats to %s", path)
    ...
    def _install_lm_eval_save_hook(cls) -> None:
        ...
        def save_results_aggregated(
            ...
            Dream._emit_inference_stats_for_eval(_ACTIVE_DREAM_LM, results)
    ...
    def _install_make_table_hook(cls) -> None:
        """Append ``dream_inference_stats`` to the CLI table printed by ``make_table``.
        ...
        def make_table(
            ...
            if column != "results":
                return table
            stats = result_dict.get("dream_inference_stats")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        global _ACTIVE_DREAM_LM
        _ACTIVE_DREAM_LM = self
...
Dream._install_make_table_hook()
Dream._install_lm_eval_save_hook()
