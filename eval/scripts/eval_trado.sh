...
tasks="gsm8k_math_verify" # "humaneval_instruct mbpp_instruct minerva_math500 gsm8k_math_verify"
nshots="5" # "0 3 4 5"
lengths="1024 1024" # "1024 1024 1024 1024"
block_lengths="4 4" # "4 4 4 4"
temperatures="0 0" # sampling temperature; default 0
alg_temps="0 0" # unmasking temperature; -1: l2r, 0: static
confidence_thresholds="0.9 0.9" # "0.9 0.9 0.9 0.9"
draft_steps="8 8" # "8 8 8 8"
...
for i in "${!TASKS_ARRAY[@]}"; do
    ...
    #     --model_args pretrained=${model},max_new_tokens=${LENGTH_ARRAY[$i]},diffusion_steps=${LENGTH_ARRAY[$i]},block_length=${BLOCK_LENGTH_ARRAY[$i]},temperature=${TEMP_ARRAY[$i]},alg_temp=${ALG_TEMP_ARRAY[$i]},early_exit=${EARLY_EXIT},use_flex_attention=${USE_FLEX_ATTENTION} \
    ...
    # echo "Output: $output_path"
    # accelerate launch eval_trado.py --model trado \
    #     --model_args pretrained=${model},max_new_tokens=${LENGTH_ARRAY[$i]},diffusion_steps=${LENGTH_ARRAY[$i]},block_length=${BLOCK_LENGTH_ARRAY[$i]},temperature=${TEMP_ARRAY[$i]},alg_temp=${ALG_TEMP_ARRAY[$i]},confidence_threshold=${CONFIDENCE_THRESHOLDS_ARRAY[$i]},early_exit=${EARLY_EXIT},use_flex_attention=${USE_FLEX_ATTENTION} \
    #     --include_path "${TASK_INCLUDE_PATH}" \
    #     --tasks ${TASKS_ARRAY[$i]} \
    #     --num_fewshot ${NSHOTS_ARRAY[$i]} \
    #     --batch_size 1 \
    ...
    echo "Output: $output_path"
    accelerate launch eval_trado.py --model trado \
        --model_args pretrained=${model},max_new_tokens=${LENGTH_ARRAY[$i]},diffusion_steps=${LENGTH_ARRAY[$i]},block_length=${BLOCK_LENGTH_ARRAY[$i]},temperature=${TEMP_ARRAY[$i]},alg_temp=${ALG_TEMP_ARRAY[$i]},decoding_alg=freedave,draft_steps=${DRAFT_STEPS_ARRAY[$i]},draft_mode=${DRAFT_MODE},eager_acceptance_mode=${EAGER_ACCEPTANCE_MODE},early_exit=${EARLY_EXIT},use_flex_attention=${USE_FLEX_ATTENTION} \
        --include_path "${TASK_INCLUDE_PATH}" \
        --tasks ${TASKS_ARRAY[$i]} \
        --num_fewshot ${NSHOTS_ARRAY[$i]} \
        --batch_size 1 \
