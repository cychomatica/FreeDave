...
# Last constructed TraDo LM (per process). Used to merge generation stats into lm_eval JSON.
_ACTIVE_TRADO_LM: Optional["Trado"] = None
...
class Trado(LM):
    ...
    def _emit_inference_stats_for_eval(
        ...
        """Merge TraDo generation stats into lm_eval output: dict key, log line, optional sidecar JSON."""
        ...
        if (
            ...
            lm_eval_results["trado_inference_stats"] = payload

        if getattr(inst, "_print_inference_stats_at_exit", True):
            eval_logger.info(
                "TraDo inference stats (summary): %s",
        ...
        if path:
            ...
            eval_logger.info("Wrote TraDo inference stats to %s", path)
    ...
    def _install_lm_eval_save_hook(cls) -> None:
        ...
        def save_results_aggregated(
            ...
            Trado._emit_inference_stats_for_eval(_ACTIVE_TRADO_LM, results)
    ...
    def _install_make_table_hook(cls) -> None:
        """Append ``trado_inference_stats`` to the CLI table printed by ``make_table``.
        ...
        def make_table(
            ...
            if column != "results":
                return table
            stats = result_dict.get("trado_inference_stats")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.block_length = kwargs.get('block_length', 4)
        ...
        if self.decoding_alg == 'freedave':
            ...
            self.eager_acceptance_mode = kwargs.get('eager_acceptance_mode', True)
        else:
        ...
        global _ACTIVE_TRADO_LM
        _ACTIVE_TRADO_LM = self
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        if self.decoding_alg == 'base':
            with self.inference_monitor.count():
                outputs = self.dlm_generation.block_decode_with_block_attention(
                    model=self.model,
    ...
    def get_logits(self, batch, prompt_index):
        """
        Block-causal TraDo needs explicit block attention and (usually) unpadded length
        multiple of ``block_length``. Dream-style models use AR-adapted logits shift
        when ``is_ar_adapted_dlm_model`` is true.
        """
...
Trado._install_make_table_hook()
Trado._install_lm_eval_save_hook()
