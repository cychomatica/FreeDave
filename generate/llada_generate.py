# adapted from https://github.com/Gen-Verse/dLLM-RL/blob/main/sample/llada_sample.py
...
def block_diffusion_generate(
    ...
    for num_block in range(num_blocks):
        ...
        x0, transfer_index = get_transfer_index(
            model_output.logits, temperature, target, mask_index_all,
...
def block_diffusion_generate_FreeDave(
    ...
    for num_block in range(num_blocks):
        ...
        x0, x0_p = sample_step(
            model, cur_x, current_block_start, past_key_values, use_cache, temperature, target
        )
        nfe += 1
        ...
        while step < cur_denoising_steps:
            ...
            if cur_denoising_steps - step > 1:
                ...
                x0_draft, x0_p_draft = sample_step(
                    model, x_draft, current_block_start, past_key_values, use_cache, temperature, target
                )
...
def token_transfer(
    ...
    # endregion
         ...
         raise ValueError(
            "Only support static strategy for now! unmask_threshold should be None, but et {} instead.".format(unmask_threshold))
