...
def block_diffusion_generate_FreeDave(
    ...
    for num_block in range(prefill_blocks, num_blocks):
        ...
        cur_attn_mask = block_diffusion_attention_mask[
            :, prefill_length:prefill_length + (cur_future_num_blocks+1) * block_length, :prefill_length + (cur_future_num_blocks+1) * block_length
        ]
        if cur_attn_mask.dim() == 3:
            cur_attn_mask = cur_attn_mask[:, None, :, :]
        ...
        if num_block < num_blocks - 1: # if not the last block
            ...
            update_attn_mask = block_diffusion_attention_mask[:, prefill_length:prefill_length+block_length, :prefill_length+block_length]
            if update_attn_mask.dim() == 3:
                update_attn_mask = update_attn_mask[:, None, :, :]
    ...
    # avg_acceptance_rate = total_accepted_steps / total_draft_steps if total_draft_steps > 0 else 0.0
    return x, total_accepted_steps, total_draft_steps
...
def sample_step(
    ...
    # Sampling
    if block_length is not None and logits.shape[1] > block_length:
        # Sample by slicing into block_length segments to align RNG with baseline
        num_tokens = logits.shape[1]
        x0_list = []
        x0_p_list = []
        for i in range(0, num_tokens, block_length):
            end_idx = min(i + block_length, num_tokens)
            x0_part, x0_p_part = sample_with_temperature_topk_topp(
                logits[:, i:end_idx],
                temperature=temperature,
                top_k=top_k,
                top_p=top_p,
            )
            x0_list.append(x0_part)
            x0_p_list.append(x0_p_part)
        x0 = torch.cat(x0_list, dim=1)
        x0_p = torch.cat(x0_p_list, dim=1)
    else:
        x0, x0_p = sample_with_temperature_topk_topp(
            logits,
            temperature=temperature,
