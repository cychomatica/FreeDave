...
# === Determinism: Disable flash_attn and liger_kernel ===
# Controlled by environment variables (set by determinism_utils.py)
# Set to "1" or "true" to enable, "0" or "false" to disable
USE_FLASH_ATTN = os.environ.get("SDAR_USE_FLASH_ATTN", "1").lower() in ("1", "true")
USE_LIGER_KERNEL = os.environ.get("SDAR_USE_LIGER_KERNEL", "1").lower() in ("1", "true")
USE_EAGER_ATTN = os.environ.get("SDAR_USE_EAGER_ATTN", "0").lower() in ("1", "true")
...
class SDARRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        # Use flash_rms_norm if available, otherwise use PyTorch native implementation
        if flash_attn_available:
            return flash_rms_norm(
                hidden_states, weight=self.weight, bias=None, eps=self.variance_epsilon)
        else:
            # PyTorch native RMSNorm (deterministic)
...
def eager_attention_forward(
    ...
    """
    Eager-mode attention forward pass using explicit matmul operations.
    
    Note: Caller (SDARAttention.forward) should already call repeat_kv on key/value
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if USE_EAGER_ATTN:
            ...
            if q_len == 1 and past_key_value is not None and flash_attn_available:
                # --- Decoding: flash-attn (only if available) ---
                ...
                # --- Use PyTorch SDPA (deterministic when flash_sdp is disabled) ---
                attn_output = F.scaled_dot_product_attention(
