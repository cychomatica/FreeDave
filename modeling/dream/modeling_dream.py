...
if is_torch_flex_attn_available():
    ...
    def _full_mask_mod(b, h, q_idx, kv_idx):
        return q_idx >= 0
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        _use_flex = is_torch_flex_attn_available() and (
            isinstance(attention_mask, BlockMask)
            or (attention_mask is None and getattr(self.config, '_generation_use_flex_attention', False))
        )
        if _use_flex:
            key_states = repeat_kv(key_states, self.num_key_value_groups)
            value_states = repeat_kv(value_states, self.num_key_value_groups)
