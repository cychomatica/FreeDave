    ...
    def forward(
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            if query_position_ids is not None:
                max_pos = max(max_pos, int(query_position_ids.max().item()))
            if key_position_ids is not None:
                max_pos = max(max_pos, int(key_position_ids.max().item()))
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        dual_layout = (
            ...
            and getattr(past_key_value, "prefix_end", None) is not None
            and getattr(past_key_value, "suffix_start", None) is not None
        )
        if past_key_value is not None:
            layer_has_kv = (
                len(past_key_value) > self.layer_id
                and past_key_value.key_cache[self.layer_id].numel() > 0
            )
            # Match HF Dream-style caches: always update when filling an empty slot or when
            # DynamicDualCache composes [prefix, window, suffix]. For a non-empty DynamicCache
            # without dual layout, concatenate prefix + current keys without persisting the
            # current slice (denoising over a long suffix after crop), same as former store_kv=False.
            if dual_layout or not layer_has_kv:
                k, v = past_key_value.update(k, v, self.layer_id, None)
            else:
                pk, pv = past_key_value[self.layer_id]
        ...
        present = (k, v) if (use_cache and past_key_value is None) else None
        query_len, key_len = q.shape[-2], k.shape[-2]  # could be different if layer_past not None

        if self.config.rope:
            # Optional explicit positions for Q/K. Tree-packed / windowed forwards use custom query ids;
            # keys must match the physical KV layout after cache concat / DynamicDualCache.compose.
            key_position_ids = None
            if query_position_ids is not None:
                pq = query_position_ids[0].long()
                if key_len > query_len:
                    if dual_layout:
                        # [prefix from base][current forward states][suffix from base]
                        pe = past_key_value.prefix_end  # type: ignore[union-attr]
                        ss = past_key_value.suffix_start  # type: ignore[union-attr]
                        base_seq = past_key_value.key_cache[self.layer_id].shape[-2]
                        suffix_ids = torch.arange(
                            ss, base_seq, device=q.device, dtype=torch.long
                        )
                        key_position_ids = torch.cat(
                            (
                                torch.arange(0, pe, device=q.device, dtype=torch.long),
                                pq,
                                suffix_ids,
                            ),
                            dim=0,
                        ...
                        # Single cached prefix (e.g. prefix DynamicCache after crop) + new tokens only.
                        key_position_ids = torch.cat(
                            (
                                torch.arange(key_len - query_len, device=q.device, dtype=torch.long),
                                pq,
                            ),
                            dim=0,
                        )
                    ...
                    key_position_ids = pq
            q, k = self.rotary_emb(
                q, k, query_position_ids=query_position_ids, key_position_ids=key_position_ids
            )
        ...
        if attention_bias is not None:
            ...
            if (
                attention_bias.dim() == 4
                and attention_bias.shape[2] == query_len
                and attention_bias.shape[3] == key_len
            ):
...
class LLaDAOutput(NamedTuple):
    ...
    attn_key_values: Optional[Union[List[Tuple[torch.Tensor, torch.Tensor]], Cache]]
    """
    Per-layer ``(key, value)`` tuples, or a shared ``Cache`` when using HF-style KV caching.
    """
...
class LLaDABlockGroup(nn.ModuleList):
    ...
    def forward(
        ...
        tuple_collect = use_cache and not isinstance(past_key_values, Cache)
        attn_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = (
            [] if tuple_collect else None
        )
        for block_idx, block in enumerate(self):
            layer_past = None
            if layers_past is not None:
                layer_past = layers_past[block_idx]
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        attention_bias: Optional[torch.Tensor] = None,
        past_key_values: Optional[Union[Sequence[Tuple[torch.Tensor, torch.Tensor]], Cache]] = None,
        use_cache: bool = False,
        ...
        query_position_ids = position_ids if position_ids is not None and position_ids.shape[1] == seq_len else None
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if am.dtype == torch.bool:
                ...
                attention_bias = torch.where(
                    am,
                    torch.zeros((), dtype=torch.float32, device=am.device),
                    torch.full((), finfo_min, dtype=torch.float32, device=am.device),
                )
            else:
                attention_bias = am.to(dtype=torch.float)
            attention_mask = None
        ...
        tuple_kv_collect = use_cache and not isinstance(past_key_values, Cache)
        attn_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = [] if tuple_kv_collect else None
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                kv_cache = past_key_values if isinstance(past_key_values, Cache) else None
                if (
                    (self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.whole_layer)
                    or (
                        self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.one_in_two
                ...
                if attn_key_values is not None and cache is not None:
                    attn_key_values.append(cache)
        else:
        ...
        out_kv: Optional[Union[List[Tuple[torch.Tensor, torch.Tensor]], Cache]] = attn_key_values
        if use_cache and isinstance(past_key_values, Cache):
            out_kv = past_key_values

        return LLaDAOutput(
            logits=logits,
            attn_key_values=out_kv,
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        kwargs.pop("store_kv", None)  # shared API with SDAR/LLaDA2; Dream-style LLaDA infers cache updates.
