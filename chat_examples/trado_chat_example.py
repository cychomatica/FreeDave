...
def main(chat_history=False):
    ...
    while True:
        ...
        if (output_ids_fast[0][len(tokens['input_ids'][0]):] == output_ids[0][len(tokens['input_ids'][0]):]).all():
            cprint('Fast generation matches normal generation', 'green')
        else:
            cprint('Fast generation does not match normal generation', 'red')
